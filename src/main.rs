...
fn main() {
    ...
    std::thread::spawn(move || {
        unsafe {
            let main_func: libloading::Symbol<unsafe extern "C" fn(api: &'static common::Api) -> !> = lib.get(b"main").expect("main() found");
            main_func(&BIOS_API);
        }
    });
...
extern "C" fn video_set_mode(mode: common::video::Mode) -> common::Result<()> {
    ...
    match mode.timing() {
        ...
        _ => {
            return common::Result::Err(common::Error::UnsupportedConfiguration(mode.as_u8() as u16));
        }
    }
    match mode.format() {
        common::video::Format::Text8x16 => {
...
extern "C" fn video_get_mode() -> common::video::Mode {
    ...
    // We know this is a valid video mode because it was set with `video_set_mode`.
    unsafe {
        common::video::Mode::from_u8(mode_value)
    }
}
...
impl MyApp {
    ...
    fn render_font(font: &font::Font, texture_buffer: &mut Vec<TextureId>, s: &mut PixState) -> PixResult<()> {
        let mut slot = 0;
        for glyph in 0..=255 {
            for palette_entry in PALETTE.iter().take(Self::NUM_FG) {
                ...
                s.with_texture(texture_id, |s: &mut PixState| -> PixResult<()> {
                    ...
                    for font_y in 0..font.height {
                        let mut font_line = font.data[((glyph as usize) * font.height) + font_y as usize];
                        for font_x in 0..8 {
                            if (font_line & 0x80) != 0 {
    ...
    fn on_update(&mut self, s: &mut PixState) -> PixResult<()> {
        ...
        for row in 0..num_rows {
            for col in 0..num_cols {
                ...
                let bg = RGBColour::from_packed(PALETTE[usize::from(bg_idx)].load(Ordering::SeqCst));
                let glyph_box = rect!(
                    i32::from(x),
                    i32::from(y),
                    8i32,
